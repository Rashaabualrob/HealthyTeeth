...
public function forget_password($user_name){
    ...
    if( $patient){
      ...
      $x=  (new TwilioSMSController)->index($patient->verification_key  ,$patient->phone  ,$patient->name , 3);
           ...
           if($initial){
               if($initial->seats > 0){
                         ...
                         (new TwilioSMSController)->initial_sms($initial  ,$patient->phone  ,$ar_name[0] ,2);
                           if($patient->bookings_num >= 4){
                            //  اذا ضل يغير بالحجز بتصله رسالة عشان ما يضل يغير
                             (new TwilioSMSController)->warning($patient->bookings_num+1 ,$patient->phone  ,$ar_name[0] ,1);
...
public function get_patient_files($access_token){
    ...
    if($patient){
        ...
        ->join('reservations','diseases.reservation_id','reservations.id')
        ->join('initials','initials.id','reservations.initial_id')
        ->select('initials.date as created_at','diseases.id')
        ->where('diseases.patient_id',$patient->id)
        ->get();
...
public function get_stars_topics($access_token){
    ...
    ->first();
   if( $patient){
    $topics=DB::table('stars')->select('id','topic')->where('status','!=','hidden')->get();
...
public function get_second_treatments(){
