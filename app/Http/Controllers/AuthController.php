...
public function set_image(Request $request)
{
      ...
      // image validation
        $validator= Validator::make($request->all(),
         ...
         if($user->image != null){
            if($user->image != 'patients/user_icon.png'){
                Storage::delete($user->image);
            }

         }

         DB::table($request->type)->where('access_token' ,'=',$request->access_token)->update(['image'=> $image])  ;
...
public function get_vedio(){
   // return response()->json(['msg'=> asset("storage").'/'.'patient2.mp4']);
    return response()->json(['msg'=> asset("storage").'/'.'Project2.avi']);
